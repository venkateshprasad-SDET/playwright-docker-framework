...
* **Docker Support**: Run tests in an isolated, headless container environment with a pre-configured Docker image.
...
├── Dockerfile                   # Docker instruction recipe
...
This framework is fully containerized using Docker, allowing you to run your tests in a clean, isolated environment without having to install browsers or dependencies locally.

For full, detailed command options and advanced overrides, see the [Docker Guide](file:///c:/Users/chara/Desktop/playwright%20practice/playwright-sample-project/DOCKER_GUIDE.md).

### Docker Quick Start:

#### 1. Build the Docker Image
Ensure Docker Desktop is running, then compile the image:
```sh
docker-compose build
```

#### 2. Run the Full Test Suite (Headless)
By default, the container runs the `npm run test` command in headless mode:
```sh
docker-compose up
```

#### 3. Run a Specific Test File (e.g., LoginTest)
You can run specific test files using Chromium inside Docker:
```sh
docker-compose run --rm -e TEST_NAME=Login -e BROWSER=chromium playwright npm run local:test
```

#### 4. Access Test Reports on your Host Machine
The `./test-results` and `./allure-results` directories are mounted as host volumes. All logs, screenshots, and videos generated inside Docker are instantly available on your local machine:
* Open Allure Report:
  ```sh
  npm run report
  ```
...
```
*(Note: Replace SheetName with your excel sheet, e.g., `Regression`)*

To run an individual test locally:
```sh
set TEST_NAME=<TestFileName> && npm run local:test
```
*(Note: Replace TestFileName with your test, e.g., `LoginTest`)*

Run an individual test in [Playwright UI Mode](https://playwright.dev/docs/test-ui-mode):
```sh
...
> [!NOTE]
> All local commands shown above are written for **Windows Command Prompt**.
